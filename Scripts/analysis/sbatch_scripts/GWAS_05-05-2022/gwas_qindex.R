...
aid_sumstats <- readRDS('outputs/version3/04_output_sumstats-function/aid_sumstats_2F_noRA.RDS')
ldsc_model <- readRDS('outputs/version3/04_output_sumstats-function/ldsc_output_04_noRA')
model_ok <- aid_model <-'F1 =~ NA*croh + uc  + psc 
            F2 =~ NA*t1d + jia + sle
F1~~F2
F1 ~~ 1*F1
F2 ~~ 1*F2
croh + uc  + psc + t1d + jia + sle ~ SNP
'

#dim(aid_sumstats)
#how_many_chunks(aid_sumstats, 10000) #432
...
saveRDS(output, file.path('outputs/version3/05_GWAS_results/GWAS_05-05-2022/q_index/chunks_qindex/', paste0(counter, '_gwas_twof.RDS')))
