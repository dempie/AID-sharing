...
a <- as.list(paste0('outputs/gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/07_colocalization/prepare_for_munge/', a1))
a<- a[-10] #remove README
a1 <- a1[-10] #remove README
names(a) <- a1 #give names

builds <- get_genome_builds(a) #check the genome build, only t1d and derma are build 38
...
format_sumstats(
 path= my_paths[[i]],
  ...
  log_folder = 'outputs/gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/07_colocalization/munged/logs/',
