...
for(i in 1:3){
  tt <- c('f1','f2', 'f3')[i]
  f_list[[tt]] <- fread(paste0('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/05_munge_for_nicola/munged/',tt,'_munged_build37.txt'), data.table = F)
...
for(i in 1:3){
  ...
  for(k in 1:nrow(to_plot_f)){
    ...
    to_plot_f[to_plot_f$leads==snp, toupper(tt)]  <-   all_f_s[str_starts(rownames(all_f_s),tt) & all_f_s$SNP==snp,]$z_score
  }
...
pdf('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/plots/figure1/heatmap_complete_absolute_z_scores_specifc_factors.pdf', height = 16, width = 6)
