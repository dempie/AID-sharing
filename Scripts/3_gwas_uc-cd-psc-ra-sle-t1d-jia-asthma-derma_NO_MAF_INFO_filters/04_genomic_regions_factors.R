...
for(i in c('f1', 'f2', 'f3')){
  ...
  sstats[[i]]<- fread(paste0('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/03_estimation/summarystats_',i,'_.txt'), data.table = F)
...
regions_factors
fwrite(regions_factors, 'outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/04_genomic_regions_factors/regions_factors.txt', row.names = F, col.names = T, sep = '\t')
