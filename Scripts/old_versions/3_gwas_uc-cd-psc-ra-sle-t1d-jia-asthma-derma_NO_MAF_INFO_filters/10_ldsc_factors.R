...
file_names <- c('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/03_estimation/summarystats_f1_.txt',
                'outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/03_estimation/summarystats_f2_.txt', 
                'outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/03_estimation/summarystats_f3_.txt')

N <- readRDS('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/03_estimation/effective_sample_size_factors.RDS')
...
munged_files <- c('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/munged/lymphocyte_count.sumstats.gz',
                  ...
                  'outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/munged/f1.sumstats.gz',
                  'outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/munged/f2.sumstats.gz',
                  'outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/munged/f3.sumstats.gz', 
                  'outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/02_sumstats_function/munged_files/t1d.sumstats.gz',
...
ldsc_output <- ldsc(traits = munged_files, 
                    ...
                    population.prev = c(NA,NA,NA,0.00001, 0.00001,0.00001 ,0.03, 0.001, 0.001, 0.000050, 0.000447, 0.000500,  0.0046, 0.035700, 0.03, 0.02,0.20), 
                    trait.names = names,
                    ld = "ldscores/eur_w_ld_chr",
                    wld= "ldscores/eur_w_ld_chr", stand = T)

saveRDS(ldsc_output, 'outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/ldsc_output.RDS')
ldsc_output <- readRDS('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/ldsc_output.RDS')
...
pdf('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/correlation_matrix_F1.pdf', height = 16, width = 16)
...
pdf('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/correlation_matrix_F2.pdf', height = 16, width = 16)
...
#-------------------------------------------------------------------------------
#plot only F3
pdf('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/correlation_matrix_F3.pdf', height = 16, width = 16)
tp <- ldsc_output$S_Stand[ c('f1', 'f2' ,'f3', 'psoriasis', 'allergies'), c( 'f1', 'f2' ,'f3',  'psoriasis', 'allergies') ]
tp['psoriasis', 'f3'] <- 1 #thje are more than 1
tp['f3', 'psoriasis'] <- 1
corrplot(tp, order = 'hclust',addCoef.col = 'black', method = 'square',type = 'upper', is.corr = T,
         tl.col='black', outline=F,
         number.cex= 1.5,
...
#plot cell couts
pdf('outputs/3_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma_NO_MAF_INFO/10_ldsc_factors/correlation_matrix_cell_counts.pdf', height = 16, width = 16)
...
ggarrange()
lympho <- ggplot(a1, aes(x=V4, y=lympho))+ geom_bar(stat='identity', width=0.5) +ylim(-0.4,+0.4)+theme_classic()
mono <- ggplot(a1, aes(x=V4, y=monocyte))+ geom_bar(stat='identity', width=0.5) +ylim(-0.4,+0.4)+theme_classic()
eos <- ggplot(a1, aes(x=V4, y=eosinophil))+ geom_bar(stat='identity', width=0.5)  +ylim(-0.4,+0.4)+theme_classic()


 ggarrange(lympho, mono, eos, ncol = 3, nrow = 1, labels = c('lymphocyte counts', 'monocyte counts', 'eosinohil counts'))

aaa + ggtitle('Genetic correlation between cell counts and factors')
