...
all_chunks <- readRDS( 'outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/03_gwas_estimation/factors_summary_stats.RDS')
...
for( i in c(1:length( list(F1_noNA, F2_noNA, F3_noNA)))){
  ...
  fwrite(to_save, paste0('outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/gwas_f',i,'_only_fuma.txt'), 
...
saveRDS(N_hat,'outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/effective_sample_size_factors.RDS')


#---plot the manatthan plot 
jpeg(file='outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/03_gwas_estimation/miamiF1_F2_F3.jpeg', width = 800, height = 400, units='mm', res = 600)
par(mfrow=c(3,1))
par(mar=c(5,5,3,3))
...
q_1_335 <- assemble_f(1:335, 'outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/qindex/', '_qindex_uc-cd-psc-ra-sle-t1d-jia-asthma-derma.RDS', 1) #The number of unique SNPs is 3324158
...
q_53 <- assemble_f(53, 'outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/qindex/chunk_53/', '_qindex_uc-cd-psc-ra-sle-t1d-jia-asthma-derma.RDS', 1)
q_93 <- assemble_f(93, 'outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/qindex/chunk_93/', '_qindex_uc-cd-psc-ra-sle-t1d-jia-asthma-derma.RDS', 1)

#assemble evetyhing together
qindex <- append_chunk(list(q_1_335, q_53, q_93),1) #3344158

saveRDS(qindex, 'outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/qindex_summary_stats.RDS')
qindex <- readRDS('outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/qindex_summary_stats.RDS')
...
saveRDS(all_chunks, 'outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/gwas_final_withQindex.RDS')

#save FACTOR GWAS individually 
for(i in c(1:3)){
  fwrite(all_chunks[[i]], paste0('outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/factor',i,'_gwas_final_withQindex.txt' ),
...
for(i in c(1:3)){
  tt <- c('f1', 'f2', 'f3')[i]
  gw[[tt]] <- fread(paste0('outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/factor',c('1', '2', '3')[i],'_gwas_final_withQindex.txt' ),
                    data.table = F)
...
#pvalue of the SNP
jpeg(file='outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/manhattan_pvalue.jpeg', width = 800, height = 400, units='mm', res = 600)
par(mfrow=c(3,1))
for(i in 1:length(g)){
  tt <- c('f1', 'f2', 'f3')[i]
  par(mar=c(5,5,3,3))
...
#Q value of the SNP
jpeg(file='outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_gwas_ouput/manhattan_qpvaule.jpeg', width = 800, height = 400, units='mm', res = 600)
par(mfrow=c(3,1))
for(i in 1:length(g)){
  tt <- c('f1', 'f2', 'f3')[i]
  par(mar=c(5,5,3,3))
