...
for(i in 1:3){
  tt <- c('f1','f2', 'f3')[i]
  f_list[[tt]] <- fread(paste0('outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/05_munge_for_nicola/munged/',tt,'_munged_build37.txt'), data.table = F)
...
pdf('outputs/2_gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/plots/figure1/heatmap_complete_absolute_z_scores_specifc_factors.pdf', height = 16, width = 6)
