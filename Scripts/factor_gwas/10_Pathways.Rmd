...
#extract the SNPs
snps <- unique(loc[loc$trait %in% c('f1', 'f2', 'f3'),]$SNP)
...
kegg <- gost(query = list(f1=unique(factor_loci[factor_loci$trait=='f1',]$v2g), 
                          f2=unique(factor_loci[factor_loci$trait=='f2',]$v2g), 
                          f3=unique(factor_loci[factor_loci$trait=='f3',]$v2g)),
             sources = c('KEGG'), 
             significant = T, 
             evcodes = T)
...
for(k in 1:nrow(factor_loci)){
  ...
  factor_loci[k, 'v2g_gene_name' ] <- genes_symbol[genes_symbol$ensembl_gene_id==factor_loci[k, 'v2g' ],'hgnc_symbol']
...
```{r}
go <- gost(query = list(f1=unique(factor_loci[factor_loci$trait=='f1',]$v2g), 
                          f2=unique(factor_loci[factor_loci$trait=='f2',]$v2g), 
                          f3=unique(factor_loci[factor_loci$trait=='f3',]$v2g)),
