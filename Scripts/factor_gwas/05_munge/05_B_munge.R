...
paths_f <- list('outputs/rev_1/05_munge/ready_munge/f1_ready_for_munge.txt',
                'outputs/rev_1/05_munge/ready_munge/f2_ready_for_munge.txt',
                'outputs/rev_1/05_munge/ready_munge/f3_ready_for_munge.txt'
...
for(i in c(1:3)){
  ...
  output[[i]] <-  format_sumstats(
    ...
    sort_coordinates = TRUE,
    nThread = 2,
    save_path = paste0('outputs/rev_1/05_munge/munged/', c('f1', 'f2', 'f3')[i], '_munged_build37.txt' ),
    write_vcf = FALSE,
    tabix_index = FALSE,
    return_data = FALSE,
    return_format = "data.table",
    ldsc_format = FALSE,
    log_folder_ind = FALSE,
    log_folder = 'outputs/rev_1/05_munge/munged/log/',
    log_mungesumstats_msgs = TRUE,
    imputation_ind = FALSE,
    force_new = FALSE,
    mapping_file = sumstatsColHeaders
