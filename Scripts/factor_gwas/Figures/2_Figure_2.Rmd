...
topl_220 <- coloc_tab[coloc_tab$pan.locus==220, c('trait', 'sub_locus', 'SNP', 'final.locus')]
topl_220

#remove the loci that are not colocalising between traits
topl_col <- topl_220[topl_220$final.locus %in% names(table(topl_220[,4])[table(topl_220[,4])!=1]),]
...
```{r paged.print=FALSE}
greg_220 <- loci.table[loci.table$pan.locus==220,]
topl$rsID <- rep(NA, nrow(topl))

for(i in topl$sub_locus){
      ll <- greg_220[greg_220$sub_locus==i, c('pJ', 'SNP')]
      topl[topl$sub_locus==i, ]$rsID <- rep(ll[order(ll$pJ),]$SNP[1], nrow(topl[topl$sub_locus==i, ]))
}
