...
shared_pathways_3 <- table(go$result$term_name)[table(go$result$term_name)==3]
kg <- go$result[go$result$term_name %in% names(shared_pathways_3),c('query', 'p_value', 'term_name', 'intersection')]
head(kg)
...
kg_10shared <- kg[kg$term_name %in% top_10_shared,]
dim(kg_10shared)
