  ...
  if(min(eqtl.results$p.value)<=1e-6){
    ...
    for(i in gene.list){   
      ...
      if(any(dataset$p.value<1e-6)){
        ...
        datasets[[i]]=dataset.munge( sumstats.file = dataset
                                    ,map = mappa.loc
                                    ,snp.lab = "SNP"
                                    ,chr.lab = "CHR"
                                    ,pos.lab = "BP"
    ...
    if(length(datasets)>1 & length(which(gene.list%in%names(datasets)))){
      ...
      for(i in 1:nrow(pairwise.list)){
        ...
        coloc.res=colo.cojo.ht(conditional.datasets[[pairwise.list[i,1]]],conditional.datasets[[pairwise.list[i,2]]],p.threshold.orig = 1e-6)
