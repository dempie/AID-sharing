...
dataset.munge=function( sumstats.file="/project/aid_sharing/AID_sharing/outputs/gwas_uc-cd-psc-ra-sle-t1d-jia-asthma-derma/07_colocalization/munged/cd_munged_build37.txt"
    ...
    if(!is.null(a1.lab) & a1.lab%in%names(dataset)){
      names(dataset)[names(dataset)==a1.lab]="A1"
    }else{
      stop("a1.lab has not been defined or the column is missing")
    }
    if(!is.null(a0.lab)& a0.lab%in%names(dataset)){
      names(dataset)[names(dataset)==a0.lab]="A2"
    }else{
      stop("a0.lab has not been defined or the column is missing")
    }
